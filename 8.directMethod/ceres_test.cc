...
struct DirectCeres
{
    DirectCeres(const cv::Mat&img1, const cv::Mat&img2 ,const Vector2d&px_ref, const double depth_ref , Sophus::SE3d&T21):
    img1_(img1),img2_(img2),px_ref_(px_ref),depth_ref_(depth_ref),T21_(T21_)
    {
    };
    ...
    bool operator()(const T*const pose,T*residual)const
    {
        ...
        Eigen::Vector3d point_ref =  depth_ref_ * Eigen::Vector3d((px_ref_[0]-cx)/fx,(px_ref_[1]-cy)/fy,1);
        ...
        residual[0] = T(double(GetPixelValue(img1_,px_ref_[0],px_ref_[1]))-double(GetPixelValue(img2_,u,v)));
                                ...
                                Sophus::SE3d &T21)
{
 ...
 SE3Parameterization* se3 =  new SE3Parameterization();
   problem.AddParameterBlock(pose,6,se3);
    for(int i=0; i<px_ref.size();i++)
    {
        ceres::CostFunction * costF = new ceres::AutoDiffCostFunction<DirectCeres,1,6>(new DirectCeres(img1,img2,px_ref[i],depth_ref[i],T21) );
      
        problem.AddResidualBlock(costF,nullptr,pose);
