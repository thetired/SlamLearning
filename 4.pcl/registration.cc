...
int  main (int argc, char** argv)
{
   ...
   pcl::io::loadPCDFile ("../bowl.pcd",*cloud_src_o);  
   PointCloud::Ptr cloud_tgt_o (new PointCloud);//目标点云
   pcl::io::loadPCDFile ("../bowl22.pcd",*cloud_tgt_o);
   ...
   pcl::io::savePCDFileASCII("bowl_src_down.pcd",*cloud_src);
...
// 同样的操作对于目标点云
   ...
   pcl::io::savePCDFileASCII("bowl_tgt_down.pcd",*cloud_tgt);
   ...
   pcl::io::savePCDFileASCII("bowl_transformed_sac.pcd",*sac_result);
   std::chrono::steady_clock::time_point sac_time = std::chrono::steady_clock::now();
   ...
   std::chrono::duration<double>sac_used = std::chrono::duration_cast<std::chrono::duration<double>>(sac_time-start);
   std::chrono::duration<double>icp_used = std::chrono::duration_cast<std::chrono::duration<double>>(end-sac_time);

   cout<<"total time: "<<time_used.count()<<" s"<<endl;
   //我把计算法线和点特征直方图的时间也算在SAC里面了
   cout<<"sac time: "<<sac_used.count()<<" s"<<endl;
   cout<<"icp time: "<<icp_used.count()<<" s"<<endl;
   ...
   //保存转换的输入点云
   pcl::io::savePCDFileASCII("bowl_transformed_sac_icp.pcd", *icp_result);
